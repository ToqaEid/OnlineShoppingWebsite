        ...
        try {
            ...
            for (FileItem item : items) {
                if (item != null && !item.getName().equalsIgnoreCase("") && item.getContentType().contains("image")) {
                    ...
                    request.getRequestDispatcher("EditProductServlet?pId="+request.getParameter("id")).forward(request, response);
                } else {
            ...
            Logger.getLogger(upload2.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(upload2.class.getName()).log(Level.SEVERE, null, ex);
        }
